  ...
  watch(() => mapStore.markers, (newMarkers: LngLat[]) => {
    ...
    for(let i = addedMarkersCount; i < newMarkers.length; i++) {
      ...
      const popup = new mapboxgl.Popup({ offset: 25 })
        .setHTML('<div>Caricamento indirizzo...</div>');
      ...
      // Ferma la propagazione del click per evitare che trigger il map.on('click')
      markerDiv.addEventListener('click', (e) => {
        e.stopPropagation();
      });

      fetch(`https://api.mapbox.com/search/geocode/v6/reverse?longitude=${item.lng}&latitude=${item.lat}&access_token=${mapbox_token}`)
        .then((response) => response.json())
        .then((data) => {
          const address = data.features[0].properties?.full_address;
          popup.setHTML(`<div>${address}</div>`);
        });

      mapStore.markerObjects[i] = newMarker; // Assegna il marker all'array nello store

      if (newMarkers.length === 1 && i === 0) {
        marker = newMarker;
      }
    }
...
map.on('click', function (e) {
    ...
    // Salva il marker nello store
    // mapStore.addMarker(lngLat);
